...
try:
    ...
    _SESSION.headers.update({
        ...
        "Cookie": "birthtime=628578001; lastagecheckage=1-0-1990; mature_content=1; Steam_Language=english",
    })
    ...
    def http_get(url, timeout=30):
        req = urllib.request.Request(url, headers={
            ...
            "Cookie": "birthtime=628578001; lastagecheckage=1-0-1990; "
                      "mature_content=1; Steam_Language=english",
        })
...
def fetch_reviews(con, appid, full=False, max_pages=200, verbose=True):
    ...
    backfill = []          # 舊資料（seed 進來的）沒有作者 steamid，順便補回去
    ...
    if backfill:
        con.executemany(
            "UPDATE reviews SET author=? WHERE id=? AND (author IS NULL OR author='')",
            backfill)
        if verbose:
            print(f"  [{appid}] 補上作者 ID {len(backfill)} 筆（供評論連結用）")
    con.commit()
    return new_rows
...
def _cell(blk, marker, width=500):
    """撈出 <div class="...marker...">…</div> 的純文字。

    不用 </div> 收尾（裡面常常還包 span），改成從標記後面切一段、
    碰到下一個 forum_topic_* 欄位就停，再把標籤剝掉。
    """
    i = blk.find(marker)
    if i < 0:
        return ""
    ...
    if j < 0:
        return ""
    tail = blk[j + 1:j + width]
    cut = re.search(r'forum_topic_(?:lastpost|last_post|reply_count|op|name|icon|overlay)', tail)
    if cut:
        tail = tail[:cut.start()]
        tail = tail[:tail.rfind("<")] if "<" in tail else tail
    return strip_tags(tail).strip()


def _replies_of(blk):
    """回覆數。Steam 有時把數字包在 span 裡，有時放 data-*，所以多試幾種。"""
    m = re.search(r'data-[a-z-]*repl[a-z-]*="(\d+)"', blk, re.I)
    if m:
        return m.group(1)
    for marker in ("forum_topic_reply_count", "reply_count", "replies"):
        txt = _cell(blk, marker, 300)
        m = re.search(r"\d[\d,]*", txt)
        if m:
            return m.group(0)
    m = re.search(r'([\d,]+)\s*(?:replies|reply|則回覆|回覆|回复)', blk, re.I)
...
def parse_steam_date(s, now=None):
    """把討論版的「最後回覆時間」轉成 YYYY-MM-DD（台北時區）。抓不到就回 None。

    Steam 會寫成 `15 Sep @ 3:42pm`、`12 Dec, 2025 @ 1:03am`、`3 hours ago` 等等。
    """
    if not s:
        return None
    ...
    m = re.search(r'(\d+)\s*(minute|hour|day|week)s?\s+ago', t)
    if m:
        n, unit = int(m.group(1)), m.group(2)
        mins = {"minute": 1, "hour": 60, "day": 1440, "week": 10080}[unit] * n
        return (now - timedelta(minutes=mins)).strftime("%Y-%m-%d")
    if "just now" in t or "moments ago" in t or "剛剛" in s or "刚刚" in s:
        return now.strftime("%Y-%m-%d")
    if "yesterday" in t or "昨天" in s:
        return (now - timedelta(days=1)).strftime("%Y-%m-%d")
    if "today" in t or "今天" in s:
        return now.strftime("%Y-%m-%d")

    m = (re.search(r'(\d{1,2})\s+([a-z]{3})[a-z]*\.?(?:,\s*(\d{4}))?', t) or
         re.search(r'([a-z]{3})[a-z]*\.?\s+(\d{1,2})(?:,\s*(\d{4}))?', t))
    if m:
        a, b, yr = m.group(1), m.group(2), m.group(3)
        dd, mon = (a, b) if a.isdigit() else (b, a)
        mon = MONTHS.get(mon[:3])
        if mon:
            try:
                y = int(yr) if yr else now.year
                d = datetime(y, mon, int(dd), tzinfo=TW)
                if not yr and d > now + timedelta(days=1):   # 沒寫年份 → 可能是去年
                    d = d.replace(year=y - 1)
                return d.strftime("%Y-%m-%d")
            except ValueError:
                pass

    m = re.search(r'(\d{4})\s*[年/-]\s*(\d{1,2})\s*[月/-]\s*(\d{1,2})', s)   # 中文/數字格式
    if m:
        return "%04d-%02d-%02d" % (int(m.group(1)), int(m.group(2)), int(m.group(3)))
    m = re.search(r'(\d{1,2})\s*月\s*(\d{1,2})\s*[日号]', s)
    if m:
        try:
            d = datetime(now.year, int(m.group(1)), int(m.group(2)), tzinfo=TW)
...
def parse_topics(html, appid):
    """把討論版列表頁解析成 [(tid, title, author, replies, url, lastpost_day)]。
    ...
    def add(tid, title, author, replies, url, last=None):
        if tid in found:
            return
        found[tid] = (tid, strip_tags(title).strip(), strip_tags(author).strip(),
                      int(re.sub(r"[^\d]", "", replies or "0") or 0),
                      url.split("?")[0], last)
        order.append(tid)
    ...
    for blk in re.split(r'(?=<div[^>]+class="[^"]*forum_topic[ "])', html):
        ...
        title = (_grab(blk, r'class="forum_topic_name"[^>]*>(.*?)</div>') or
                 ...
                 _cell(blk, "forum_topic_name"))
        author = (_grab(blk, r'class="forum_topic_op"[^>]*>(.*?)</div>') or
                  _cell(blk, "forum_topic_op", 200))
        last = parse_steam_date(_cell(blk, "forum_topic_lastpost", 400) or
                                _cell(blk, "forum_topic_last_post", 400))
        add(m.group(2), title, author, _replies_of(blk), m.group(1), last)
    ...
    if not found:                      # 退路：至少把標題跟連結撈出來
        for m in url_re.finditer(html):
            ...
            title = (_grab(tail, r'forum_topic_name[^>]*>(.*?)</div>') or
                     _grab(tail, r'>([^<>]{4,120})<'))
            add(m.group(2), title, "", _replies_of(tail), m.group(1),
                parse_steam_date(_cell(tail, "forum_topic_lastpost", 400)))
...
def fetch_threads(con, appid, pages=4, verbose=True, debug_dir=None):
    ...
    for p in range(1, pages + 1):
        ...
        rows += [(t[0], appid, t[1], t[2], t[3], t[4], now, t[5]) for t in topics]
        if verbose:
    ...
    # 解析品質自我檢查：一串都沒抓到、或回覆數／日期全是空的，就把現場留下來對照
    bad = (not rows or all(r[4] == 0 for r in rows) or all(r[7] is None for r in rows))
    if bad and first_html is not None and debug_dir:
        try:
            os.makedirs(debug_dir, exist_ok=True)
            blk = ""
            parts = re.split(r'(?=<div[^>]+class="[^"]*forum_topic[ "])', first_html)
            for q in parts:
                if "/discussions/" in q:
                    blk = q[:2500]
                    break
            with open(os.path.join(debug_dir, "_threads_debug.txt"), "w",
                ...
                f.write(f"# {datetime.now(TW):%Y-%m-%d %H:%M} appid={appid} "
                        ...
                        f"reply_count={first_html.count('reply_count')} "
                        f"lastpost={first_html.count('lastpost')}\n\n"
                        f"---- 第一個討論串區塊 ----\n{blk}\n\n"
                        f"---- 頁首 3000 字 ----\n{first_html[:3000]}")
        except Exception:
            pass
    ...
    changed = []
    for tid, aid, title, author, replies, url, seen, last in rows:
        old = prev.get(tid)
        kind = delta = None
        # 日期優先用 Steam 自己寫的「最後回覆時間」，抓不到才退回今天
        act_day = last or today
        if act_day > today:
            act_day = today
        if old is None:
            # 第一次看到：沒人回過 → 那天是開串日；有回覆 → 那天是最後回覆日
            kind, delta = ("new" if replies == 0 else "reply"), replies
        ...
        con.execute(
            ...
            (tid, aid, title, author, replies, url,
             int(datetime.strptime(act_day, "%Y-%m-%d").replace(tzinfo=TW).timestamp())
             if last else seen,
             seen))
        if kind:
            con.execute(
                ...
                (tid, act_day, kind, delta, replies))
    con.commit()
    return changed
...
def cmd_seed(args, con):
    ...
    for line in open(args.file, encoding="utf-8"):
        ...
        rows.append((str(r["id"]), args.appid, r.get("lang"), 1 if r.get("up") else 0,
                     ...
                     r.get("txt", ""), r.get("au") or None, now))
    ...
    with open(out, "w", encoding="utf-8") as f:
        for r in rows:
            f.write(json.dumps({
                ...
                "au": r["author"] or "",          # 作者 steamid → 組評論永久連結用
                "txt": r["text"],
            }, ensure_ascii=False) + "\n")
            n += 1

...
TOPIC_RE = re.compile(
    ...
    r'href="(?P<url>https://steamcommunity\.com/app/\d+/discussions/\d+/(?P<tid>\d+)/?[^"]*)".*?'
    r'class="forum_topic_name"[^>]*>(?P<title>.*?)</div>.*?'
    r'(?:class="forum_topic_op"[^>]*>(?P<author>.*?)</div>)?.*?'
    r'(?:class="forum_topic_reply_count"[^>]*>\s*(?P<replies>\d+))?',
    re.S)
...
def fetch_threads(con, appid, pages=4, verbose=True):
    ...
    for p in range(1, pages + 1):
        ...
        for m in TOPIC_RE.finditer(html):
            rows.append((m.group("tid"), appid, strip_tags(m.group("title")),
                         strip_tags(m.group("author")), int(m.group("replies") or 0),
                         m.group("url"), now))
            found += 1
        if verbose:
            print(f"  [{appid}] 討論版第 {p} 頁：{found} 串")
        if found == 0:
            break
        time.sleep(1.0)
